...
function castCreate(statuses, conf) {
  ...
  var seed = (string, cx) => fnMem('seed', seedClasses)(string, statuses, cx || {});
  var funcs = fnMem('seedAll', cachedArgs, function(displayName, castName, classes) {
    return {
      displayFn: seed(displayName, classes),
      displayFnPlain: seed(displayName),
      castFn: seed(castName, classes)
    };
  })(displayName, castName, classes);

  return fnMem('allCreate', cachedArgs, fnMem('create', cachedArgs, function(n, sx) {
    var fns = [];
    ...
    return fnMem('render', cachedArgs, renderClasses)(fns, n, sx);
  }));
}
